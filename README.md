# dragnote

## В linux запустить fluidsynth:
- Установить ALSA в ОС.
- Установить в ОС ```fluidsynth```.
- Открыть терминал и запустить ```fluidsynth```.
- Узнать номер синтезатора ```aconnect -l``` (номер - это порядковый номер из приведенных, начиная с 0)
- Вписать номер синтезатора в ```settings.yaml```

## Запуск:
- ```python dragnote```

## Добавить кучу гамм: https://samesound.ru/write/78409-piano-scales-for-dummies

## Ошибки:
- 2 раза при успехе на упражнении играет буль-буль, а должно быть один.
- Введение в одной строке нескольких нот не работает
- Неверное выполнение при ошибке, неверный звук. Гейм глючит.
- first note нужно переделать на вывод: либо first note, 
либо список уже угаданных нот